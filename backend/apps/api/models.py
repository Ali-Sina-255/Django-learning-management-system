...
class Teacher(models.Model):
    ...
    image = models.FileField(
        upload_to="course-file", blank=True, null=True, default="course.jpg"
    )
...
class Course(models.Model):
    ...
    teacher_course_start = models.CharField(
        max_length=255, choices=TEACHER_COURSE_START_CHOICES
    )
    platform_status = models.CharField(max_length=255, choices=PLATFORM_STATUS_CHOICES)
    course_id = ShortUUIDField(
        unique=True, max_length=30, length=6, alphabet="1234567890"
    )
...
class CartOrder(models.Model):
    ...
    payment_status = models.CharField(
        choices=PAYMENT_STATUS, default="pending", max_length=10
    )
...
class CartOrderItem(models.Model):
    order = models.ForeignKey(
        CartOrder, on_delete=models.CASCADE, related_name="order_items"
    )
